...
function getContextWithFilters(widget, accountId, parent, initialContext) { 
  ...
  <div className="flex items-center justify-center h-[70vh]">
    <div className="flex flex-col items-center gap-4">
      <svg
        className="w-12 h-12 animate-spin text-[#1EA88A]"
        fill="none"
        viewBox="0 0 24 24"
      >
        <circle
...
const DataExplorer = ({
  ...
  const renderQuickStats = () => {
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-[#ffffff]/60 backdrop-blur-md border border-slate-200 rounded-xl p-4 hover:border-[#1EA88A]/20 transition-all duration-200">
          <div className="text-xs text-gray-400 mb-1">Total Records</div>
          <div className="text-2xl font-bold text-slate-800">{totalCount?.toLocaleString() || '0'}</div>
        ...
        <div className="bg-[#ffffff]/60 backdrop-blur-md border border-slate-200 rounded-xl p-4 hover:border-[#1EA88A]/20 transition-all duration-200">
          ...
          <div className="text-2xl font-bold text-slate-800">{allColumns?.length || '0'}</div>
        ...
        <div className="bg-[#ffffff]/60 backdrop-blur-md border border-slate-200 rounded-xl p-4 hover:border-[#1EA88A]/20 transition-all duration-200">
          ...
          <div className="text-2xl font-bold text-[#1EA88A]">{selectedIndices?.size || '0'}</div>
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-300">
      ...
      {/* Aggregates Panel */}
      {showAggregates && (
        <div className="mb-6 p-4 bg-[#ffffff]/60 backdrop-blur-md rounded-xl border border-slate-200 shadow-lg z-[100]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {numericColumns && numericColumns.map(col => (
              <div key={col} className="bg-[#e7f2ed] p-3 rounded-lg border border-slate-200">
                <div className="text-sm text-gray-400 truncate">{col}</div>
                <div className="text-lg font-bold text-slate-800">{aggregates[col]?.sum?.toLocaleString()}</div>
      ...
      {showStats && (
        <div className="mb-6 p-4 bg-[#ffffff]/60 backdrop-blur-md rounded-xl border border-slate-200 shadow-lg z-[100]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-[#e7f2ed] p-3 rounded-lg border border-slate-200">
            ...
            <div className="bg-[#e7f2ed] p-3 rounded-lg border border-slate-200">
              ...
              <div className="text-2xl font-bold text-slate-800">{numericColumns?.length || '0'}</div>
            </div>
            <div className="bg-[#e7f2ed] p-3 rounded-lg border border-slate-200">
      ...
      {/* Controls Bar */}
      <div className="flex flex-wrap items-center gap-3 mb-4 p-4 bg-[#ffffff]/50 backdrop-blur-md rounded-xl border border-slate-200 shadow-lg">
        {/* Search */}
        <div className="relative flex-1 min-w-[200px] max-w-md">
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-[#f8faf9] border border-slate-200 rounded-lg text-slate-800 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#1EA88A] focus:border-transparent transition-all"
          />
        ...
        {/* View Mode Toggle */}
        <div className="flex bg-[#f8faf9] rounded-lg border border-slate-200 p-0.5">
          <button
            onClick={() => setViewMode("table")}
            className={`px-3 py-1.5 text-sm rounded-md transition-all ${
              viewMode === "table"
                ? "bg-[#1EA88A] text-white shadow-sm"
                : "text-gray-400 hover:text-slate-800 hover:bg-white/5"
            }`}
          >
            List
          </button>
        ...
        <button
          ...
          className={`px-3 py-2 bg-[#f8faf9] border border-slate-200 rounded-lg text-sm transition-all ${
            showAggregates
              ? "text-[#1EA88A] border-[#1EA88A]/50 bg-[#1EA88A]/10"
              : "text-gray-400 hover:text-slate-800 hover:border-slate-300"
          }`}
        >
        ...
        <button
          ...
          disabled={loading}
          className="px-4 py-2 bg-[#1EA88A] hover:bg-[#188f76] text-white rounded-lg text-sm flex items-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-[#1EA88A]/20"
        >
      ...
      {Object.keys(columnFilters).length > 0 && (
        <div className="flex flex-wrap gap-2 mb-4">
          {Object.entries(columnFilters).map(([key, value]) => (
            <div
              key={key}
              className="flex items-center gap-2 px-3 py-1.5 bg-[#1EA88A]/20 text-[#1EA88A] rounded-lg text-sm border border-[#1EA88A]/30"
            >
              <span>{key}: {value}</span>
              <button
      ...
      {/* Table Container */}
      <div className="bg-[#ffffff]/50 backdrop-blur-md rounded-xl border border-slate-200 shadow-xl overflow-hidden">
        <div className="overflow-auto max-h-[600px]">
          <table className="min-w-full border-collapse text-xs text-left">
            <thead className="bg-[#e7f2ed] text-gray-400 font-bold sticky top-0 z-[10] shadow-lg">
              <tr>
                {/* Selection Header */}
                <th className="px-4 py-3 sticky left-0 z-[50] bg-[#e7f2ed] border-b border-r border-slate-200 w-[50px] text-center">
                  <input
                    type="checkbox"
                ...
                {visibleColumns.map((column) => (
                  <th
                    key={column}
                    className="px-4 py-3 sticky top-0 z-[40] bg-[#e7f2ed] border-b border-slate-200 cursor-pointer hover:text-slate-800 transition-colors group"
              ...
              {showFilterRow && (
                <tr className="border-t border-slate-200">
                  <td className="px-4 py-2 sticky left-0 z-[50] bg-[#e7f2ed]"></td>
                  {visibleColumns.map((column) => (
                    <td key={column} className="px-4 py-2 sticky top-0 z-[40] bg-[#e7f2ed]">
                      <input
                        type="text"
                        placeholder={`Filter ${column}...`}
                        value={filterInputs[column] || ""}
                        onChange={(e) => handleColumnFilterChange(column, e.target.value)}
                        className="w-full px-2 py-1 bg-[#f8faf9] border border-slate-200 rounded text-slate-800 text-xs focus:outline-none focus:ring-1 focus:ring-[#1EA88A]"
                      />
                    </td>
                  ))}
                </tr>
            ...
            <tbody className="divide-y divide-white/5">
              {visibleData.map((row, index) => (
                ...
                >
                  <td className="px-4 py-2 sticky left-0 z-30 bg-[#ffffff] border-r border-slate-200">
                    <input
                      type="checkbox"
                    ...
                    return (
                      ...
                      >
                        ...
                        {showDataBars && typeof numericValue === 'number' && !isNaN(numericValue) && (
                          <div className="absolute left-0 top-0 bottom-0 bg-[#1EA88A]/10 rounded-r"
      ...
      <div className="flex items-center justify-between mt-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3 py-1.5 bg-[#f8faf9] border border-slate-200 rounded text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-white/5 transition-all"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <span className="px-3 py-1.5 text-sm text-gray-300 bg-[#f8faf9] border border-slate-200 rounded">

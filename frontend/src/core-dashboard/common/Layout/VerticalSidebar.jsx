...
export default function VerticalSidebar({
  ...
  useEffect(() => {
    const count = parseInt(
      localStorage.getItem(uploadCountStorageKey) || "0",
      10,
    );
  ...
  const NavItem = ({ item }) => {
    ...
    return (
      <NavLink
        ...
        className={({ isActive }) => `
          group flex items-center justify-center lg:justify-between gap-0 lg:gap-3
          px-2 lg:px-3 py-2.5 lg:py-2 mb-1 rounded-lg
      ...
      >
        <div className="flex items-center gap-0 lg:gap-3">
          <item.icon size={20} className="lg:w-4 lg:h-4" />
    ...
    <>
      ...
      <AnimatePresence mode="wait">
        {showTooltip && hoveredItem && (
          ...
          >
            <div className="bg-[#121214] border border-white/10 rounded-xl shadow-2xl overflow-hidden w-[280px]">
              <div className="px-4 py-2.5 flex items-center gap-2 border-b border-white/10">
                <hoveredItem.icon size={14} className="text-white/80" />
      ...
      {/* Sidebar (dark + clean, no glow/shadows) */}
      <div
        className="fixed top-0 left-0 h-screen w-[72px] lg:w-[240px] border-r border-white/10 z-50 flex flex-col transition-all duration-300"
        style={{ backgroundColor: "var(--brand-secondary, #007758)" }}
      >
        {/* Brand */}
        ...
        <div className="flex-1 overflow-y-auto px-2 lg:px-3 space-y-6 scrollbar-hide">
          {groups.map((group, index) => (
            <div key={index}>
              <p className="hidden lg:block px-3 text-[10px] font-bold text-white/70 uppercase mb-2">
                {group.title}
              </p>
        ...
        {/* Footer Upload (optional) - clean, no glow */}
        {showFooterUpload && (
              ...
              <div className="flex flex-col items-center gap-2">
                {isLocked ? (
                  ...
                  <button
                    ...
                    onClick={() =>
                      onUploadClick ? onUploadClick() : navigate("/upload")
                    }
                  >

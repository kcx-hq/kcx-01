...
const CloudFileManagerPanel = ({ cloudConfig }) => {
  ...
  const handleSelectFile = async (filePath) => {
    ...
    try {
      ...
      setActionMessage(
        res?.data?.message || "File selected for ingestion successfully.",
      );
    } catch (err) {
      setActionMessage(
        err?.response?.data?.message || err?.message || "Failed to select file.",
      );
    } finally {
      setIngestingPath("");
    }
  ...
  return (
    <div className="w-full">
      ...
      <div className="mb-3 flex items-center gap-2 text-xs text-gray-400 flex-wrap">
        <button
          type="button"
          onClick={() => loadPath("")}
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                ...
                {files.map((file) => {
                  ...
                  const isBusy = ingestingPath === file.path;
                  return (
                    <tr
                      key={file.path}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right">
                        <button
                          ...
                          disabled={isBusy}
                          className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-[#a02ff1] hover:bg-[#8e25d9] text-white disabled:opacity-60"
                        >
                          {isBusy ? "Selecting..." : "Select file for ingestion"}
                        </button>
                      </td>
                    </tr>
                  );
      ...
      {actionMessage ? (
        <div className="mt-3 px-3 py-2 rounded-lg border border-white/10 bg-white/5 text-xs text-gray-300">
          {actionMessage}
        </div>
      ) : null}
    </div>
